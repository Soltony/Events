...
import { getRoleById, updateRole } from '@/lib/super-admin-user-actions';
        ...
        if (fetchedRole) {
          ...
          if (Array.isArray(fetchedRole.permissions)) {
            ...
            try {
              ...
              if (str.includes(',')) {
                currentPermissions = str.split(',').map((s: string) => s.trim()).filter(Boolean);
              } else if (str) {
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          setPermissionCategories(json.permissions || {});
        }
      } catch (e) {
  ...
  if (loading) {
    return (
      <div className="flex flex-1 flex-col gap-4 md:gap-8">
        ...
        <Card>
          <CardHeader><Skeleton className="h-7 w-48 mb-2" /></CardHeader>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                <FormField control={form.control} name="name" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="e.g., Event Manager" {...field} disabled={role?.name === 'Admin' || role?.name === 'Super Admin'} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="description" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="Briefly describe this role's purpose" className="resize-none" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                ...
                <div className="space-y-8">
                  ...
                  {administrativeEntries.length > 0 && (
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-lg font-bold">Administrative Modules</h3>
                        <p className="text-sm text-muted-foreground">Full Create, Read, Update, and Delete permissions per module.</p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {administrativeEntries.map(([category, actions]) => renderPermissionCard(category, actions))}
                      </div>
                    </div>
                  )}

                  {featureAccessEntries.length > 0 && (
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-lg font-bold">Feature Access Modules</h3>
                        <p className="text-sm text-muted-foreground">Simple on/off access to a page or feature.</p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {featureAccessEntries.map(([category, actions]) => renderPermissionCard(category, actions))}
                      </div>
                    </div>
                  )}
                </div>
