...
export default function PublicHomePage({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
  const [events, setEvents] = useState<EventWithTickets[]>([]);
  const [homeCarouselAds, setHomeCarouselAds] = useState<
    { id: number; imageUrl: string; title: string | null; caption: string | null; linkUrl: string | null }[]
  >([]);
  ...
  useEffect(() => {
    async function fetchData() {
        setLoading(true);
        try {
            const [fetchedEvents, ads] = await Promise.all([
              getPublicEvents(),
              getPublicHomeCarouselAds(),
            ]);
                        ...
                        <ul>
                          {searchSuggestions.map(event => (
                            <li key={event.id}>
                              ...
                              >
                                <Image 
                                    src={getPrimaryEventImage(event.image) || DEFAULT_IMAGE_PLACEHOLDER}
                                    alt={event.name} 
                                    width={40} 
                                    height={40} 
            ...
            <div className="container mx-auto px-4 lg:px-6">
                ...
                {loading ? (
                    ...
                    upcomingEvents.map((event) => {
                      ...
                      const hasFreeTicket = event.ticketTypes.some((ticket) => Number(ticket.basePrice) === 0);
                      return (
                          ...
                          <CardBody className="bg-white relative group/card w-full h-full rounded-xl p-0 border border-black/[0.1] flex flex-col justify-between">
                            <CardItem translateZ="50" className="w-full">
                               <div className="relative w-full h-[200px] bg-muted rounded-t-xl overflow-hidden">
                                  ...
                                  {hasFreeTicket && (
                                    <div className="pointer-events-none absolute left-[-2.7rem] top-3 z-10 w-32 rotate-[-35deg] bg-[#f4b619] py-1 text-center text-[11px] font-extrabold tracking-[0.16em] text-black shadow-lg sm:left-[-2.9rem] sm:top-4 sm:w-36 sm:text-xs">
                                      FREE
                                    </div>
                                  )}
                               </div>
