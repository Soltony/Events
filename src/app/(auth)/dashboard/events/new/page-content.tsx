...
export default function CreateEventPageContent() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload all selected files and append to existing images.
      ...
      form.setValue('images', [...prev, ...uploadedUrls], { shouldValidate: true });
      toast({ title: 'Image(s) added', description: `${uploadedUrls.length} image(s) uploaded.` });
    } catch (error: any) {
      toast({
        variant: 'destructive',
        title: 'Upload failed',
      ...
      // Let the same file be re-selected immediately if needed.
      e.target.value = '';
    }
  };
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-4 flex-wrap">
                      {watchedImages.map((imageUrl, index) => (
                        <div key={`${imageUrl}-${index}`} className="relative aspect-video w-64 rounded-md overflow-hidden group">
                          <Image
                            src={imageUrl}
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <Button
                              ...
                              variant="destructive"
                              size="icon"
                              onClick={() => form.setValue('images', watchedImages.filter((_, i) => i !== index), { shouldValidate: true })}
                            >
                              <Trash2 className="h-4 w-4" />
                      ...
                      <label
                        ...
                        className="aspect-video w-64 rounded-md border-dashed border-2 flex items-center justify-center cursor-pointer hover:border-primary hover:text-primary transition-colors text-muted-foreground"
                      >
                        <div className="text-center">
                            ...
                            <>
                                ...
                                <span className="text-sm mt-2">{watchedImages.length > 0 ? 'Add More Images' : 'Add Images'}</span>
                            </>
                            )}
                        </div>
                        <Input
