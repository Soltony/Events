...
import { CheckCircle2, Download, Calendar, MapPin, Ticket as TicketIcon, User } from 'lucide-react';
        ...
        <div className="container mx-auto py-8 md:py-12 max-w-lg">
            <Card className="shadow-2xl rounded-2xl overflow-hidden bg-card/90 backdrop-blur-sm border-primary/20">
                <CardHeader className="text-center items-center bg-primary/10 p-6 space-y-3">
                    ...
                    <CardTitle className="text-3xl font-bold text-primary-foreground">Thank You!</CardTitle>
                    <CardDescription className="text-lg text-muted-foreground">Your ticket has been confirmed.</CardDescription>
                </CardHeader>
                <CardContent className="p-6 md:p-8">
                    <div className="flex flex-col items-center space-y-4">
                        <div className="p-3 border-4 border-muted rounded-lg bg-white shadow-md">
                          {qrCodeDataUrl && <img id="qr-code-image" src={qrCodeDataUrl} alt="Ticket QR Code" className="w-48 h-48 md:w-56 md:h-56" />}
                    ...
                    <div className="space-y-4 text-sm">
                        <h3 className="text-xl font-semibold text-center mb-4">{ticket.event.name}</h3>
                        <div className="flex justify-between items-center">
                            <span className="text-muted-foreground flex items-center gap-2"><User className="h-4 w-4" /> Attendee</span>
                            <span className="font-semibold">{ticket.name}</span>
                        </div>
                         <div className="flex justify-between items-center">
                            <span className="text-muted-foreground flex items-center gap-2"><TicketIcon className="h-4 w-4" /> Ticket Type</span>
                            <span className="font-semibold">{ticket.ticketType.name.split(' - ')[0]}</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-muted-foreground flex items-center gap-2"><Calendar className="h-4 w-4" /> Date</span>
                            <span className="font-semibold text-right">{formatEventDate(ticket.event.startDate, ticket.event.endDate)}</span>
                        </div>
                        <div className="flex justify-between items-center">
