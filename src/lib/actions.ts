...
export async function getPublicHomeCarouselAds() {
    const ads = await (prisma as any).homeCarouselAd.findMany({
        where: { isActive: true },
...
export async function createHomeCarouselAd(data: {
    ...
    const maxRow = await (prisma as any).homeCarouselAd.aggregate({
        _max: { sortOrder: true },
    });
    const nextOrder =
    ...
    const ad = await (prisma as any).homeCarouselAd.create({
        data: {
...
export async function updateHomeCarouselAd(
    ...
    const ad = await (prisma as any).homeCarouselAd.update({
        where: { id },
        data: payload,
    });
    revalidatePath('/');
...
export async function deleteHomeCarouselAd(id: number) {
    await requireAdmin();
    await (prisma as any).homeCarouselAd.delete({
        where: { id },
    });
    revalidatePath('/');
    revalidatePath('/dashboard/settings/home-ads');
